...
| 提示 `DESKTOP_BRIDGE_UNAVAILABLE` | 文件传输前没有发现可用的新版 Xshell 桌面桥接会话；在任一已打开的 Xshell 标签页重新运行 `xshell_agent_bridge.py` 后再试。 |
...
- 编码或下载内容直接交给 `sh` / `bash`，以及 Agent 发起的 Shell、Python、Node、PowerShell 等脚本解释器、`eval`、`source`、命令替换也会被拒绝，避免未经审阅的第二阶段命令执行。静态文本策略不能替代服务器最小权限账号、受限 `sudoers`、文件权限、备份或经审核的命令白名单；企业服务器应继续使用这些系统级保护。
